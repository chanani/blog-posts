...
라이브러리를 구축했지만 독립된 서버에서 라이브러리를 실행시키기 위해 제한된 언어와 파이프라인 구축을 서버 생성 시
...
이미 검증된 액션을 활용한다면 제가 고민하던 **언어 제약** 문제를 한 번에 해결할 수 있을 것이라는 확신이 들었습니다.
...
이후 AI 코드 리뷰를 사용하는 레포지토리에서 공유 레포지토리를 주입 받아 설정을 진행했습니다.(uses 항목에서 공유 전용 레포지토리 호출)

### 결과

이렇게 모듈화를 진행한 결과, 새로운 프로젝트에 AI 리뷰를 도입하는 과정이 **복사 붙여넣기**에서 **참조**로 바뀌었습니다.
매 프로젝트마다 30~50줄의 yml 설정할 필요없이 공유 전용 레포지토리를 호출함으로서 동료 개발자들의 불편함을 개선할 수 있었습니다.
